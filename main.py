...
def diff(obj1, obj2, path, results):
    if isinstance(obj1, dict) and isinstance(obj2, dict):
        keys = set(obj1) | set(obj2)
        for k in keys:
            p = f"{path}.{k}" if path else k
            if k not in obj1:
                results.append((p, "<missing>", json.dumps(obj2[k], separators=(',', ':'))))
            elif k not in obj2:
                results.append((p, json.dumps(obj1[k], separators=(',', ':')), "<missing>"))
            else:
                diff(obj1[k], obj2[k], p, results)
    else:
        if obj1 != obj2:
            results.append((path, json.dumps(obj1, separators=(',', ':')), json.dumps(obj2, separators=(',', ':'))))
